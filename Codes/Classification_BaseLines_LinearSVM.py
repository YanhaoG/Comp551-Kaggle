...
typeHyperParameter = 'C'
hyperParameter = [0.001, 0.01, 0.1, 0.5, 1]  #C in SVM
...
for k in range(kFold):
    ...
    for item  in hyperParameter:
        ...
        SVM_clf = LinearSVC(C=item, verbose=0)
        SVM_clf.fit(X_train, t_train.ravel()) 
        
        t_train_predicted[:,k] = SVM_clf.predict(X_train)
        t_valid_predicted[:,k] = SVM_clf.predict(X_valid)
        t_test_predicted[:,k]  = SVM_clf.predict(X_test)
